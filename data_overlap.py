...
OUTPUT_BASE_DIR = r"dataset\processed_data_label_overlap"
...
def avg_resp_rates(n_process_window, tester_id, state, window_idx=0):
    ...
    # 4. 平均後的呼吸率為最終輸出
    resp_rate = np.mean(resp_list)

    prev_resp_rate = resp_rate

    logging.info(f"Calculated averaged respiration rate: {resp_rate:.2f}, raw respiration rate:{resp_rate_raw}, bpm for tester {tester_id}, state {state}, window {window_idx}")
